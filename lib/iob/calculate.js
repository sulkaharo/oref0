...
function iobCalc(treatment, time, dia, profile) {
    ...
    if (curve == 'bilinear') {
    	if (!logged) { console.error('Calculating IOB using bilinear model, dia=' + dia); logged = true; }
    ...
    if (t < td) {
	    ...
	    if (!logged) { console.error('Calculating IOB using exponential_curves, td=' + td + ', tp=' + tp); logged = true; }
